...
| cred-obtained | add-cred kind=session | 身份差分候选（add-intent kind=authz-diff 直达 pending）+权限面复测 | 逐对：authz-diff 且 cred=<CRED-id> 的 intent 或延后 fact（target=authz-diff:<CRED-id>）（G-27 列落地，批5 兑现） |
...
> → triggers-v2 机检兑现（批5 T6：②逐对配对+④高危横向进 trigger-audit 检查面 3→5——
> 目录行集合与语义零变，仅消费检查列文字升级为已兑现机检口径，版本不 bump）。
