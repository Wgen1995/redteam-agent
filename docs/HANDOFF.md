...
2026-09-24｜子代理 T8｜批次5 T8：P4 攻击链落证步（G-28 后半）——duty 第 5 步命令序列全文（graph-paths --to=scope-root → stdout 存 evidence/attack-paths.txt 只增不覆盖重跑 -r2 后缀 → add-evidence capture+artifact+repro_command=命令原文+raw_excerpt=首路径行；双指纹由命令自算=norm.py 单源 LLM 不手算）——attack 链从图推导升格可审计证据；test_attack_paths_ev 2 例双钉 TDD 先红后绿（红=duty 文本缺位；集成走查=diff-authz 端到端落盘→add-evidence→validate/verify-chain/hash-recheck 全过+同路径 REJECT/-r2 放行），全套 455→457 绿+金样 51 面 PASS 零漂移（graph-paths 既有面未动）｜5b4daf0
