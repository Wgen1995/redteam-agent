...
> 🧭 **导读**：欢迎进内网——你手里只有 VOL-01/02 交过来的那只低权限 shell，正站在别人的机器上，两眼一抹黑。
> 先别急着打谁：这一节是整条攻击链的“入口审计”，像当年接手代码先通读路由与配置，先弄清这台机器是谁、归谁管、在不在域。
> 这份档案收不全，后面的隧道、提权、横向每一步都在靠猜。
...
> 🧭 **导读**：本机档案有了，但这只 shell 只够你趴在原地——想够到内网其他机器，得先修路。
> 回头翻 1.2 的 `ipconfig` 与 `netstat` 输出：网卡和现有连接里，写着这一节怎么选的全部依据。
> 类比审计：这步相当于给目标系统挂上你的代理，此后对内网的每个动作都从这条通道走。
...
> 🧭 **导读**：隧道通了，脚也伸进内网了——但“登得进去”不等于“带得走资本”，这条链上的硬通货是凭据。
> 这节的活儿你最熟：找硬编码密钥、翻配置、捞历史记录，只是仓库从代码库换成了内存、注册表和浏览器。
> 每捞到一枚哈希都收好，第 5 节的 PTH 和票据攻击全拿它们当门票。
...
> 🧭 **导读**：揣着一兜凭据却还是低权限账户——第 3 节已经让你撞过一次“需 SYSTEM”的门槛，往前走这样的门槛只会更多。
> 现在的目标只有一个：把 shell 换成 SYSTEM/root，否则第 5 节步步是门禁。
> 节奏提醒：这一节的动作最容易把机器打崩，授权业务在跑时，先挑稳路走。
...
> 🧭 **导读**：单机 SYSTEM 到手——恭喜拿下第一个据点，但整张网还在别人手里，旅程刚过半。
> 从这节起视角要换挡：从“这台机器上有什么”切到“这张网怎么连”，像审计时从单个函数跳出来追整条调用链。
> 前几节攒下的哈希、明文、私钥，从现在开始逐张兑现。
...
> 🧭 **导读**：脚下换赛道了——到这里为止都是工作组思路“拿到凭据就去登”，从这一页起进入域的协议世界。
> 这页别跳过：5.3 到 5.5 的每一把刀都落在这几步认证流程里，先看懂地图，才知道刀口该指哪。
> 用你审计的老习惯来读：先摸清协议状态机，再找缺校验的那一环。
...
> 🧭 **导读**：一路杀到山顶——横向已经让你能触到域控，整场旅程只差这临门一脚。
> 打 DC 没有唯一正解：这节按“你手里攥着什么资本”来选打法，动身前先盘点第 5 节攒下的钥匙。
> 把它当地图而非教程，之后到第 8 节的靶场里，你会反复回来对号入座。
...
> 🧭 **导读**：域控到手，攻击旅程收官——从现在起你从攻击者切回工程师身份，进入下山与对账段。
> 节奏提醒：这一节的清单每条都是边界，动手前重读授权书范围，宁可少删、不可多碰。
> 转岗前你交审计报告，转岗后交的还是报告——只是证据从代码 diff 换成了这条攻击链本身。
