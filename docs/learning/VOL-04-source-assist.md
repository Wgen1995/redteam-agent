...
> 🧭 **导读**：折算表看完，你现在站在全卷最熟悉的地形上——手里有源码。这一章最终只交付一样东西：一张黑盒可以直接下手的打点清单。带着这个目标走，§1 结束时你的 Burp 里就该躺着第一批待发请求了。
...
> 🧭 **导读**：从这页起，源码被收走了——这是全卷最大的一次换挡。审计时你从 main 一路往下读，现在得倒着来：用报错、前端 JS、指纹三块碎片，把当初那份调用图拼回去。别慌，你读堆栈的速度，就是别人手里的源码。
...
> 🧭 **导读**：§2.1、§2.2 你一直在做苦力——重建服务端真相。翻翻刚攒下的产出：err1.txt 里的包名、endpoints.txt 里的组件痕迹，它们就是下一节的钥匙。从这里不再逐条拼图，而是换赛道：指纹一命中，直接跳到现成的 CVE 与 PoC。
...
> 🧭 **导读**：§2 刚被收走的源码，这一章会还你一部分——"无源码"经常是伪命题，客户官网或许就挂着真相的快照。为什么是现在：§2.3 指纹只给了你版本区间，正需要一份真实代码来对账，两条线在这里合流。
...
> 🧭 **导读**：到这里，三条路你都已走过一遍：翻译、反推、半黑盒。最后这章不加新武器，只解决一人红队的真瓶颈——发现隔天换机还能复现。你写了五年"复现步骤"的手感，就是这一章全部的底子。
