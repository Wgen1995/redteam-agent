...
> 🧭 **导读**：第一站是 Windows 安装包：EXE 这条线会把「判壳→解包→提取→反编译」四步走全，第 3 节的 APK 不过是换一套工具重跑同一套流程。
> 节奏提醒：判壳（2.2）没过之前，你别急着对样本跑 strings——对着外壳和加壳产物提取，就像审计时硬读压缩包里的 .class，白费功夫。
...
> 🧭 **导读**：到这站画风一变：前面你都在「拆开看」，从这里开始「读代码」——.NET 反编译几乎还原源码，你五年源码审计的手艺第一次可以原样平移。
> 观察引导：2.4 攒下的凭据表里那些「看得出是密钥、看不出怎么用」的常量，钥匙就在这节——全局搜索定位加密类，回溯它。
...
> 🧭 **导读**：换月台了：你刚在 Windows 走通的四步流水线整体平移到 Android——apktool/jadx 对位 7z/dnSpy，「加固」对位「加壳」，先判加固再定策略，道理同 2.2。
> 节奏提醒：别被新名词唬住，路径没变；手生时回第 2 节找对应步骤，对照着走。
...
> 🧭 **导读**：全卷最大的转轨在这站：前面你一直在「拆包看死的产物」，现在要让客户端跑起来、看它怎么说话——静态转动态。
> 为什么是现在：你手里已有地址清单（2.4/3.3）、可读源码（2.5/3.1），可能还摸到了密钥——三样凑齐，才有本钱把报文字节和代码函数一一对号，协议这才「半透明」。
...
> 🧭 **导读**：这站不是新技术，是收口：把前面所有「疑似凭据」压成一张经得起 challenge 的 finding 卡——证据链的套路和你写审计报告同源，只是证据从代码行换成了样本偏移。
> 节奏提醒：先过五要素再下笔；缺一项，评审会上你就只剩「我看到了一个字符串」。
