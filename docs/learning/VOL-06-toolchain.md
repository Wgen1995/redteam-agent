...
> 🧭 **导读**：把这卷当工具字典，别从头读——按任务查组、按工具查条目，每个条目固定“定位/原理/操作/验证/陷阱”五段，像你当年翻 SAST 规则手册那样按需取用。急着动手就直接跳第 6 节战术矩阵，按场景反查该串哪几件工具；无论跳到哪，“陷阱”一段都值得停 30 秒。
...
> 🧭 **导读**：你站在整条链路的最前端——后面四章每条命令要填的 IP 和 URL，全指望这组的产出。像审计开工先画入口点清单：此刻的目标是“门摸全”而不是“破门快”，这组工具个个能打爆告警，节奏就是你的授权额度。
...
> 🧭 **导读**：侦察给你的还是一张静态清单，从这组起你要逐扇门上手了。把 Burp 的 HTTP history 当 Web 世界的调试日志：每个请求-响应对都是可重放、可篡改的一次调用——你审计时读调用链的直觉，在这张表里直接变现。
...
> 🧭 **导读**：上一组的 Repeater 里应该已攒下几个可疑点——这组负责把它们变成敢写进报告的实锤。心态回到审计岗：工具输出只是告警线索，判定永远归你；任何命中不经手工复核，就只是误报候选人。
...
> 🧭 **导读**：能翻到这组，说明你已有立足点——从这章起每个动作都发生在客户内网，授权边界从“这台机器”收紧到“每一步横向”。节奏提醒：别急着上 C2 框架，一人红队先把“能落地、能留证、能收场”三件事练熟，MSF 加隧道已覆盖大多数授权场景。
...
> 🧭 **导读**：别被“辅助”二字骗了——前四章攒下的发现，能不能变成客户签字的报告，取决于这一组。把你写审计工作底稿的习惯搬过来：证据当场落、PoC 可复现、数据不出本机；工具解决“找到”，这组解决“交付”。
