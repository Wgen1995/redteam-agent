...
**日期**：2026-09-21 · **性质**：综合设计定稿（唯一权威设计文档）
**取代**：TanYin 原稿（docs/design/imported/TanYin/ 全套：REQUIREMENTS.md / doc/2026-09-17-tanyin-design.md / doc/2026-09-18-web-blackbox-engine.md / architecture.md / PRINCIPLES.md）与方案 B′（docs/research/2026-09-19-ai-pentest-landscape.md §7 十二条 must-haves）。与两者冲突处，**以本文为准**。
**上游**：docs/design/2026-09-20-tanyin-bprime-fusion.md（融合裁决，下称**融合文档**）——本文是其 §6 蓝图经用户拍板（§0 ADR-P1~P4）后的落地定稿；融合文档 §6.5 十个拍板点全部关闭。
**证据缩写**：A. = .research/tanyin/A-core-design.json；B. = .research/tanyin/B-principles-panorama.json；L§ = landscape 章节；P§ = pain-points 章节；T. = .research/dive/tsecbench-q2-2026.json。
...
### 0.1 本次用户拍板四条（最高约束，全文不可违背）

| ADR | 决策 | 内容 | 对设计的直接后果 |
|---|---|---|---|
| **P1** | 路线：融合出 v2 | TanYin 完整设计为骨架 + B′ 实证门禁嫁接 | 融合文档十大裁决（§0.2）全部生效；TanYin A1-A20/B1-B17 审计落点继承（4 项改落点见融合文档 §5.5） |
| **P2** | 宿主：五宿主一起进首批安装矩阵 | DSH / opencode / codex / walcode / CodeBuddy | §10 安装矩阵五宿主首批；TanYin「双宿主」承诺与 B′「三后端」并列合流；验收含 walcode/CodeBuddy 验证盲区方案 |
| **P3** | 宪法：接受「skill 主体 + 薄 CLI 工具箱 + 档位化 hook/egress」 | **推翻 TanYin 纯零代码 D 决议**（守卫脚本不做） | §2 铁律 5/7 新增；账本命令实现载体 PS 5.1 → python3 标准库薄 CLI；四层执法分档（§8.5）；「LLM 只调用不实现」「凡未给出命令的步骤不得执行」两条真纪律原样保留 |
| **P4** | 其余七点按融合文档 §6.5 建议执行 | ①TSV 外置卡片 ②python3 only ③身份矩阵批次 0 契约、批次 4 落地 ④egress 默认开、DSH 可降档 ⑤弱模型档位化、默认档覆盖不可谈判 ⑥预算 $ 维度默认关 ⑦evals 三层验收含 TSecBench 对齐 | 分别落位于 §4.11、§2 铁律 7、§11 批次 0 与批次 4、§8.5、§8.8/§2 铁律 6、§8.7、§9 |

**推翻依据（P3，引用融合文档 §5.1）**：①TanYin 账本命令本就是 PowerShell 5.1 片段（A.`skillOrganization.howZeroCodeWorks`），「零代码」实指零运行时依赖，字面表述早已名存实亡；②PS 5.1 是 Windows 基底，与五宿主（macOS/Linux 为主）直接冲突，且 A.`openItems` 自认其性能边界是探知项；③实测反例（Threatswarm fail-open + `$TARGET` 绕过，P§P1.5.2）证明纯 skill 层执法拦不住子代理直连界外目标，expert/ai-agent #3 判「法律级风险」。

### 0.2 融合十大裁决一览表（全部生效；#4/#10 为对 TanYin 的推翻项）

| # | 裁决点 | v2 裁决（本文落位） | 一句话理由（出处） |
|---|---|---|---|
| 1 | 总体形态 | skill 主体 + 单一薄 CLI 工具箱 + 按宿主档位启用 hook/egress | 双方独立同路；Claude Code 零渗透代码排 5/13、多阶段 41.1% 第二（T.`rankings`） |
| 2 | 状态账本 | **TSV 13 表保留为领域账本**（TanYin 12 表 + 新增 creds 表，§4.2 口径裁定）；timeline.tsv 兼任 journal（第一事实源+可全量重建）；嵌套走「TSV 索引行 + 外置卡片」双轨 | 换 JSONL 不解决状态机/并发两病灶（A.决策6）；TSV 满足 B′ 全部动因 |
| 3 | 阶段门 | phases.yaml 数据化状态机承载九门语义；**执法权威仍在账本命令**（§5） | 形式归 B′、语义归 TanYin（R2） |
| 4 | scope 执法 | **四层纵深分档**（账本级→执行通道级→宿主 hook→egress 代理）+ canary 进 evals；**推翻 TanYin「不做守卫脚本」D 决议**（ADR-P3） | 事前可拒收≠事前不可绕过（A.决策7 honestBoundary）；Threatswarm 实洞 |
| 5 | 凭据治理 | `{{vault:cred-N}}` 唯一占位符语法 + 全链路四关卡 + 输出兜底重 tokenize + withheld 降级不拒绝（§8.4） | 同物异名统一（R3）；B′ 链路更全（DarkMoon 模式） |
| 6 | 预算 | 三元组树化（goal 根、intent 叶）+ 可选 $ 第四维**默认关** + 假设排序公式并入先验分（§8.7） | 同基准 token 差 30 倍（T.`headline.token_variance`） |
| 7 | 证据契约 | E-index 扩展 POC 四要素 + P4 新增独立重放门（三态）；findings 吸收 auth_context / exploitation_status（§4.9/§6.1/§6.6） | 「复现四要素只是口号」级缺口（expert/pentest #2） |
| 8 | 验收体系 | 三层：黄金夹具逐字节 → evals CI（含 token 效率）→ TSecBench 六域对齐（§9） | 互补不重叠（R5） |
...
继承 TanYin 七条核心诉求 D1-D7（A.`coreDemands`），v2 仅对 D1 与形态条款修订，其余语义不变：

| 需求 | TanYin 原文要点 | v2 修订标注 |
|---|---|---|
| D1 通用可复用 | 直接安装到 walcode、CodeBuddy 等使用 | **修订**：首批安装矩阵 = DSH / opencode / codex / walcode / CodeBuddy 五宿主（ADR-P2）；复制即装承诺不变，但「装」= 幂等安装器（§10.1）建立权威目录 + 符号链接 + tools.lock 校验 |
| D1.1 形态条款 | 纯 SKILL：零代码、零运行时、复制即装 | **修订（ADR-P3）**：skill 主体（markdown+数据文件）+ 单一薄 CLI 工具箱（python3 ≥3.9 标准库实现，零第三方依赖，tools.lock 锁定哈希+ECDSA 验签）+ 按宿主档位启用的 hook/egress。「零代码」重述为「零第三方运行时依赖、复制即装」 |
| D2 全类型渗透 | 不止 Web，编排多引擎 | 不变（引擎契约 §6；首批 web-blackbox + vuln_agent） |
| D3 不断增强 | 知识跨会话沉淀复利 | 不变（§7；新增 B′ 语料入库与 CVE 联网核验） |
| D4 图谱驱动 | 像真人头脑风暴——侦测信息→分析→攻击面演进→推导验证，图谱收敛=测试完成 | 不变并恢复 v1 原话（§5/§4；十边口径钉死；探索语义总纲见 §5.4） |
| D5 安全纪律 | 授权门+分级审批+命令安全上下文 | 不变并**加强**（§8：四层执法分档 + canary + 占位符四关卡） |
| D6 覆盖可度量 | 五态标记、矩阵闭合、终态门禁 | 不变（§8.8 弱模型档位化以「覆盖不可谈判」为不可裁剪底线，ADR-P4⑤） |
| D7 工程化管理 | 蓝图→设计→进度→账本 | 不变（本文即蓝图定稿；批次推进见 §11） |
| 验收（req §7） | 黄金夹具 / 授权靶场 / 82 漏洞 ingest 抽查 / 双宿主可用 | **修订**：双宿主 → **五宿主安装矩阵验证**（§10，含盲区静态验证+手测脚本）；其余三条并入三层验收体系（§9） |

**不做的边界（继承并修订）**：宿主插件、独立二进制、多 agent 运行时、GUI（烛龙是 GUI 未来）、多 session 并行（首发单 session 串行，graph.ndjson 行级合并预留）、业务逻辑漏洞全自动发现（biz 标记+人工为主）。原「不做执行沙箱/守卫」修订为「沙箱仍不做、**守卫以执行通道级 CLI + 宿主 hook + egress 分档形式做**」（ADR-P3）。
...
四条 TanYin 铁律原文保留（B.`iron_laws`；cross_view #8 消解——全文收录），新增三条 v2 铁律。**铁律高于一切机制设计；机制与铁律冲突时改机制。**

### 2.1 铁律 1-4（TanYin 原文保留）
...
### 2.3 铁律 6（新增）：默认档覆盖不可谈判（弱模型档位化的边界）
...
### 2.4 铁律 7（新增）：薄 CLI 边界定义（哪些能力允许进 CLI 工具箱）
...
**禁止进 CLI**：攻击决策与假设生成、漏洞语义判定、任何对「是否漏洞/下一步测什么」的判断、知识提炼（ingest 语义层）、报告执行摘要与修复建议叙述。这些永远是 LLM+skill 的职责。**CLI 不主动发起对外请求**——唯一例外：tanyin-replay 与 tanyin-egress 在授权窗口与 scope ACL 约束下执行/转发。「LLM 只调用不实现」「凡涉及账本读写而未给出命令的步骤一律不得执行（视为技能缺陷，终止报告）」两条纪律原样保留。

### 2.5 词典口径（cross_view #1 消解，v2 强制重命名）
...
L1 知识纪律层 knowledge/ 飞轮（+B′ 语料经 staging 入库）+ shared/ 六件
             （DISCIPLINE/EVIDENCE/SEVERITY/LEDGER/VOCAB/DENYLIST）
             + scope（accounts/oob/amendments）+ 四层执法档位表
```

B′ 三层与本架构对位：胶水层=L3+L4 契约；门禁层=L1 执法档位+CLI 内置校验；契约层=L3 schema+POC 卡片+报告模板。
...
- **薄 CLI 工具箱**=确定性与执法面：一切「计算、校验、拦截、重建」发生在 cli/。总控 LLM 经宿主执行通道调用 CLI（命令签名附录 A 冻结，LLM 只调用不实现）；CLI 是账本唯一写入口（写前拒收内置）。**咬合点 1**：phases/*.md 中每条指令引用的命令一律经「执行通道」命名（当前=宿主 shell 直通 `tanyin-ledger <cmd>`；沙箱化只换通道实现，phases 不改——继承 B17 执行通道间接层）。
...
- **咬合点 3**：安装自检（B14 扩展）同时决定执法档位与机制分档，探测结果写 goals.guard_tier / model_tier，报告披露。
...
- **Harness**：复杂性放基础设施且用现成的——不 fork 宿主、不做运行时；子代理并发/命令执行/权限控制复用宿主；v2 修订：允许随工具箱分发 python3 薄 CLI 与可选 egress 代理组件（ADR-P3），仍不做 agent 运行时。
...
### 3.4 目录结构（安装区与交战区分离——B′ devops #1 吸收）
...
规则：交战区与知识库**永不在 skill 安装树内**（git pull/升级不冲突、状态不混居程序文件）；knowledge/ 与 SKILL 代码分离布局带 format_version，版本不匹配拒绝恢复并提示迁移命令；artifacts 运行时以 `submissions/<intent-id>/artifacts/` 为准（cross_view #9 消解）。
...
- 文件：TSV，UTF-8 无 BOM + LF，写命令内部钉死编码；全表带 `schema_version` 列（v2 定稿值 = `2`）。
...
- **ID 铸造**（A1）：`{前缀}-{goal-id}-{四位序号}`，定宽零填充，字典序=时间序；由 ledger-next-id 原子分配；子代理/引擎无铸造权。前缀表：G(goal)/S(scope)/INT/F(fact)/FD(finding)/AST(asset)/E(edge)/AP(approval)/EV(evidence)/CRED(cred)/PG(pair_group)。
- **单写者**：所有写操作由总控串行执行；子代理/引擎只产 submissions/<intent-id>/submission.json，总控验收后落账。
- **事件溯源**（A3）：intents/matrix/creds 状态变更=追加新行（同 id 多行），「取最新」是账本命令；finding 合并=supersedes 边+tombstone，不删行。**写前拒收**：每条写命令自带行级校验（列数/ID 格式/枚举/转义/引用闭合/dedup_key 唯一），畸形 REJECT 不部分写入。
- **journal 职能**（R1）：timeline.tsv 是第一事实源，全部 13 表可由 timeline+提交文件全量重建；新增命令 ledger-state-rebuild 校验 state.md 与账本重建结果一致（kill -9 半写兜底：state 写入走 temp/rename 原子替换并带 revision 号）。
...
| 7 | edges.tsv | 纯追加 | ledger-add-edge | 不变（10 边） |
| 8 | approvals.tsv | 纯追加 | ledger-approve | 不变 |
| 9 | E-index.tsv | 纯追加（只增不覆盖） | ledger-add-evidence | +network_position/card_path；四要素进卡片 |
| 10 | matrix.tsv | 事件溯源（长表） | ledger-matrix-set（matrix-init 生成） | 不变；authz 差分落标准格 |
...
> **口径裁定 CB-1（与融合文档措辞差异，以本文为准）**：融合文档 §0-9/§6.1 表述为「12 表 + creds/sessions」。v2 将 session 落地为 creds.tsv 的 `kind=session` 行：会话令牌本质是短时效凭据，字段同构（秘密入 vault、生命周期、获取来源），拆两表将重复校验逻辑且账本变 14 表、破坏 13 表口径。身份矩阵差分按 `kind+role` 分组重放（§6.6）。凡本文写「creds/sessions 实体」处均指 creds.tsv 的两类行。
...
### 4.4 scope.tsv（授权白名单 + B′ schema 吸收，R7）
...
- `oob`=OOB 回连白名单端点（DNS/HTTP 回连接收方申报——expert/architect #3；**未申报的回连被 Tier 3 默认拒绝且记 fact**）。
...
- engine=目标引擎名；kind∈{recon,surface,matrix-test,deep-dive,**authz-diff**}（引擎段映射 §6.3）；origin∈{entity,concept,precedent,adjacency,llm,recon-event,mixed}；score=先验分 0-1（命令计算，公式见 §8.7）；via=命中知识页引用；dedup_key=资产+技法类（命令机械计算，重复键 REJECT——LLM 只提议不判重）；**budget_share=`token;requests;hours[;dollars]`**（预算树叶节点，R4）；activation=结构化谓词 `field;op;value`（deferred 用，命令评估）；reason=状态变更原因（rejected/blocked/deferred 强制）。
...
### 4.8 assets.tsv / edges.tsv / approvals.tsv（继承）
...
- source_type∈{command,capture,file,log,manual}；**network_position**∈{internet,intranet,same-host,jumphost:<name>}（POC 四要素之一进列——网络位置声明是中文报告被质疑复现不了的第一大原因，expert/pentest #2）；repro_command 第三方可跑（凭据一律 `{{vault:cred-N}}` 占位符）；repro_kind∈{single,sequence,concurrent}（时序类引用 artifact 内并发脚本）；content_hash 双轨（raw+normalized：归一化去 nonce/时间戳后哈希）；artifact_path 只增不覆盖（重跑另存 -r2）；pair_group=差分组；raw_excerpt 脱敏+定长截断；card_path→外置证据卡片（嵌套四要素部分，§4.11）。
...
- **timeline.tsv**：`timestamp, actor, phase, event, revert_cmd, prev_hash, hash`——第一事实源+审计链+清理台账三职合一；actor∈{总控,子代理,CLI,人工}；**revert_cmd**=该写操作的逆操作命令（R9：所有写操作强制登记；无逆操作者填 `irreversible` 并强制 L3 逐条审批）；prev_hash+hash 链式哈希（**哈希输入=本行全部字段含 revert_cmd**——改任何历史行即断链可见）；对外请求记 `request:` 事件；P0 落账 SKILL 版本+tools.lock 哈希；managed-restart 事件记 spawn 方式（auto/manual）。
...
### 4.11 findings 外置卡片契约（B′ 六字段映射；ADR-P4①「TSV 索引+外置卡片」双轨）
...
---
id: FD-g1-0001
dedup_key: AST-g1-009+wstg-authz-bola          # B′ 字段①：命令计算，与 TSV 列同值
scope_check: in_scope                            # B′ 字段②：与 TSV 列同值
exploitation_status: verified                    # B′ 字段③：与 TSV 列同值，重放门维护
confidence: C1                                   # B′ 字段④：四级（C1/C2/C3/➖🛑）
impact: 高
auth_context: CRED-g1-0003                       # B′ 字段⑤：与 TSV 列同值
control_evidence_ids: [EV-g1-0042]               # B′ 字段⑥：≡control_evidence_ids（统一命名）
...
**EV 卡片**（evidence/EV-{id}.md，承载 POC 四要素嵌套部分，R6）：
...
**映射规则**：B′ 六字段中一切标量进 TSV 列、一切嵌套/富文本进卡片 front-matter，两处以 id/card_path 互链；**聚合器以 TSV 列为权威，卡片与 TSV 不一致=P4 ledger-validate 失败**（同值性校验）。
...
- phases.yaml 是**阶段序列、entry/exit 断言、门禁点、回边的单一事实源（声明层）**；TanYin 九门业务语义全部数据化。**执法权威不搬家**：每条 exit 断言=一条（或一组）账本命令的调用与返回值判定——yaml 只声明「调哪条命令、期望什么返回」，判定由命令执行，yaml 自身不可被执行为旁路。
...
gates:
  ...
  P5:
    ...
    duty: "tanyin-report 聚合器从 13 表确定性重建正文（模板+账本数据，零 LLM 方差；LLM 仅写执行摘要
           ...
           out_of_scope facts 列示供客户扩授权决策——B4 恢复，接 §5.4 边界生长闭环）"
    exit:
      assert:
...
补充语义：P0「无授权只许读文档」由 Tier 0 硬门实现（无 goals 行时一切写命令 REJECT）；受管重启自动档护栏=重启计入预算、重启速率上限（1 次/N 分钟防递归 spawn）、timeline 记 managed-restart 事件、单活跃会话约束；工件即缓存幂等续跑=intent done 且 `submissions/<intent-id>/submission.json` 存在则重入跳过（R8 吸收）。

### 5.3 命令集（37 条 = TanYin 31 条签名冻结原样 + 新增 6 条）

- **写命令 18**：add-goal / add-scope / add-intent / set-intent-status / add-fact / add-finding / supersede-finding / add-asset / add-edge / add-evidence / approve / matrix-set / checkpoint / append-timeline / matrix-freeze / budget-log + **新增** add-cred、amend-scope。
- **查询命令 12**：unconsumed-facts / pending-intents / matrix-gaps / converge-check / next-id / intent-status / matrix-get / scope-check / budget-check / cleanup-checklist + **新增** set-cred-status（creds 事件溯源状态查询/变更）、redact-scan（交付前终检）。
- **校验命令 6**：validate / verify-chain / hash-recheck / matrix-audit + **新增** state-rebuild（state.md 与账本重建一致性）、set-replay-state（重放门三态 VERIFIED/REPAIRED/REJECTED 落账，REJECTED→confidence 降 C3 或转 fact）。
- **特殊**：matrix-init（P2 生成矩阵）。合计 18+12+6+1=**37**。
- 通用纪律（继承）：写前拒收；查询输出摘要化（计数+top-N+ID 列表，禁全量回灌）；命令幂等；对外请求类前置 request-ticket。实现载体=cli/tanyin-ledger（python3 标准库）；签名清单批次 0 冻结进 shared/LEDGER.md 附录 A。

---

### 5.4 探索迭代语义总纲：图谱生长与锚点冻结（头脑风暴不变式）

> 设计意图：恢复 D4 原话「像真人头脑风暴——侦测信息→分析→攻击面演进→推导验证」。P0-P2 只铸三类锚点（授权边界/词表/矩阵基线），**攻击面生命周期=整个交战**；测绘不终于 P1，规划不锁死探索。本节把散在 §4.4/§4.10/§5.2/§6.6/§8.7 的生长机制收拢为一张总表——「开局不定死」是显式设计承诺，不是机制副产品。
...
**边界生长闭环（B4 恢复）**：范围外观察 → P5「范围外观察」附录列示 → 客户决策扩授权 → amend-scope 修订链 → egress recompile+canary 复测 → 界外资产复判转正 → 子矩阵生成。授权边界的变更是留痕闭环，不是重启交战。
...
**执行语义三 kind**：`skill`=LLM 子代理执行（失败=格式漂移/上下文耗尽，超时=回合预算）；`cli`=OS 进程执行（失败=非零退出/超时/部分产物；**必须声明纪律能力**：max_op_level+视角上限，总控路由拒绝超限 intent，黑盒工具默认最高风险级、仅 L0/L1 视角 intent 可派；适配器输出操作日志供审计回放 B13）；`projector`=只读账本、产物不回写。超时/重试/幂等声明于 manifest。契约内零改动（满足契约即插即用；契约演进引擎跟版本）；烛龙接入只换 runner（§11 批次 7）。
...
### 6.6 身份矩阵差分子流程（B′ 独有吸收；契约批次 0 定死、实现批次 4，ADR-P4③）
...
| 语料 | 入库路径 | 形态 | 门 |
|---|---|---|---|
| CNPEN 82 漏洞五类素材（测试全景图/思路复盘/测试记录 T1-T55/31 份黑盒漏洞单/BurpPOC 合集） | 全景图→夹具素材与矩阵样例；复盘八阶段+7 条核心思路→concepts 方法论原则（errorCode 语义分析/前端 JS 是 API 说明书/微服务直连假设）；测试记录→precedents 先例链路（脱敏）；漏洞单→patterns/submission-ok 样例+技法判定标准（注入深挖后才定级）；BurpPOC→技法页工具参数 | A.`webBlackboxEngine.cnpenMaterialMapping` | 同一道 staging 审批+反向验证；**词表基线保持 WSTG 全集**（防 CNPEN 纯 Java 样本过拟合——TanYin 自身教训） |
| nuclei 模板库 + PortSwigger WSA 分类 | concepts 技法页素材 + VOCAB 多元化佐证 | 词表基线多元化（A7） | staging |
| B′ 语料（均 MIT）：BugHunter hunt-* 模板骨架、Threatswarm 27 agent 语料、CEP 知识束与 report-template.html | hunt-*→concepts 技法页骨架；27 agent 语料→concepts/precedents 素材；CEP ROE amendments→scope 模板语义；report-template.html→聚合器报告模板底版 | R11 | **同一道 staging 审批门**（防投毒对称性：外部语料与自有语料同闸） |
| CVE 联网核验 | 技法页 lint 规则：涉及具体 CVE 的模式必须经 WebSearch 对照 PSIRT/NVD/CISA KEV 后更新 last_verified——**明确不信任训练数据** | B′ CEP 纪律 | lint 强制；未核验条目带 stale 标记，风暴检索降权 |
...
### 8.5 四层 scope 执法（档位表；R7 + ADR-P3/P4④）
...
| Tier 2 | 宿主 hook | 宿主命令拦截点 fail-closed（Threatswarm exit-2 语义修补版：阻断并返回非零+timeline 记录） | 宿主有 hook 机制即强制启用（§10.2 差异） |
...
- 最低保证：Tier 0+1 恒在=「账本级硬门+执行通道硬门」双层不可绕过底线；Tier 2/3 缺席档位的残余暴露面（本机其他进程直连界外）在报告声明中明示——诚实边界表述从 TanYin 三层升级为**四层+档位事实**（铁律 5）。
- scope 全量吸收 B′：accounts+permitted_actions、oob_endpoints 申报、append-only amendments（§4.4）。
...
## 9 验收体系（三层，全部 CI 门禁化；R5 + ADR-P4⑦）

### 9.1 L1 黄金夹具（确定性层：账本命令逐字节回归）

固定盘上样本 session（13 表+证据+卡片+矩阵，钉死编码）→ 命令输出规范化比对（剥时间戳/ID 重映射/行排序稳定化）→**字节级一致**；两层：确定性规范化比对+行为结构断言（该拒的拒了/该晋升的晋升了）。引擎级夹具：固定 intent+固定技法页片段→提交文件与 submission-ok 结构一致（字段全/凭据占位符化/证据双哈希/pair_group 标注）。**迁移验收（批次 1）**：PS 5.1 语义→python3 CLI 的夹具回归先于一切业务功能。canary/kill-9/复放率三项 B′ 硬指标并入 L2。

### 9.2 L2 交战级 evals（CI 门禁；改动即跑，不绿不合入）

| 指标 | 门 | 来源 |
|---|---|---|
| 金标精确率/召回率（docker-compose 靶场+ground truth） | ≥基线，回退即 fail | B′ |
| POC 机器复放率（重放门三态分布） | C1 finding 100% 可重放或已降级处置 | B′ |
| **scope canary 零容忍（Tier 0-3 各档位分别跑）** | 任一界外触达=fail | B′+R7 |
| kill -9 续跑保真度（随机断点恢复后账本/矩阵/state 无损） | state-rebuild PASS | B′+R8 |
| **token 效率：每闭合一个矩阵格的 token 成本** | 对标 CHYing 量级（15.78M/85.14% 参照），超阈值告警 | TSecBench |
| 纪律注入红队集（敌意工具输出/诱导越权/绕账本写） | 系统行为不变 | TanYin C4 升格 |
| 负向用例（无授权跑 P0-P2→timeline 零主动命令；界外资产喂 add-intent→REJECT） | 必须失败 | TanYin |
| 弱模型档遵循率（最低档模型跑协议负向用例） | 「未给出命令的步骤终止报告」可检测 | 融合 §5.2 |
| 身份矩阵差分检出率（靶场种认证后漏洞） | ≥基线 | B′+融合 |
| 报告 schema lint+脱敏检查（redact-scan 零泄漏） | PASS | B′ |
| 机制开关矩阵（强/弱档×开关组合冒烟） | 铁律 6 不可裁剪清单不破 | 融合 §5.2 |

退出码对齐 Strix 0/1/2（0=通过/1=门禁失败/2=环境问题可重跑）。

### 9.3 L3 外部基准对齐（TSecBench 六域）

对齐 TSecBench 六能力域分类法（Web 漏洞挖掘/二进制漏洞挖掘/漏洞利用/**多阶段渗透**/云攻击/对抗规避），**多阶段渗透为主指标**（全场瓶颈 29.53%、完整解题率 8.33%，T.`headline`；榜首 Cairn 多阶段 44.6% 的黑板+事实-意图图结构与本设计同构，T.`rankings`[0]；本设计重状态账本正瞄准此短板）。跑分口径：三轮取优（能力上限）+token 均值同时报告（CHYing 27.92M vs Cairn 458.50M 的 30 倍差距教训）。L3 为发布前外部对齐项，不阻塞日常 CI。
...
- **双知识库抽查**：CNPEN 82 ingest 后抽查（判据：字段完整/指纹可检索/无跨客户残留）+ B′ 语料（BugHunter/Threatswarm/CEP）入库抽查（同判据+CVE 核验标记齐全）。
...
| **walcode** | 其技能目录符号链接（TanYin 原稿宿主；headless：walcode run/serve，B.`context_strategy` 记载实测可用） | 按其 hook/权限机制探测挂载；无则 Tier 1+披露 | 默认 Tier 3（组件可运行时）；否则降档披露 | 夹具/干跑/headless 自动档/受管重启演练 | **验证盲区**（无环境，§10.3） |
| **CodeBuddy** | 其技能/规则目录符号链接（TanYin 原稿宿主） | 按其机制探测；无则 Tier 1+披露 | 同上 | 夹具/干跑/常驻集注入验证（系统级 vs 会话级） | **验证盲区**（无环境，§10.3） |
...
| **5 知识飞轮+语料入库** | staging/lint/四门槛/三元组/CLIENT-NN/graph.ndjson+CNPEN 82+B′ 语料（BugHunter/Threatswarm/CEP）入库+CVE 联网核验 | 双知识库抽查通过（§9.4）；反向验证零命中 | 技法页/先例页 front-matter schema；词表版本化 |
...
## 12 残余风险登记册（融合七项 + v2 新增五项；逐项缓解与残余）

| # | 风险 | 等级 | 缓解 | 残余 |
|---|---|---|---|---|
| R1 | 弱模型对 37 命令协议遵循率未验证（模型间差 20+pt，T.`headline.best_model`） | 高 | 机制分档（§8.8）+evals 开关矩阵+负向用例（未给出命令的步骤终止报告可检测）；**首发限定模型档位清单** | 首发清单外模型组合未背书 |
| R2 | 账本命令箱 PS 5.1→python3 迁移引入实现 bug | 中 | 黄金夹具字节级回归先行（批次 1 验收）；夹具覆盖度即语义漂移防线 | 夹具未覆盖分支仍可能漂移 |
...
| **R8（新）** | 五宿主首批验证风险：walcode/CodeBuddy 无环境实测，hook 挂载点/常驻集注入/子代理并发行为未知 | 中 | §10.3：静态验证六项（CI 宿主无关）+用户手测脚本回传机制；该两宿主默认保守披露 Tier 1；DSH 侧全链路实测先行 | 首个实测回传前的兼容性声明带「待实测」标记 |
| **R9（新）** | egress 默认开带来安装摩擦（代理组件依赖/网络受限环境），用户倾向跳过 | 中 | 安装器自动探测：可承载→Tier 3；不可承载→显式降档确认+披露；DSH 之外宿主降档不阻塞安装但入守门声明 | 降档交战的暴露面教育成本长期存在 |
| **R10（新）** | canary 误报（诱饵域名被本机其他进程/CDN 解析触碰） | 低 | canary 判定绑定「本交战进程凭证+时间窗」非裸 DNS 查询；误报进 evals 基线校准 | 极端共享环境仍有告警噪音 |
| **R11（新）** | 报告确定性重建的模板法务审查未做（免责条款/等保占位段合规表述） | 低 | 批次 6 出口前人工法务过审一次；模板版本化留痕 | 各客户法务差异需交付时二次确认 |
| **R12（新）** | 身份矩阵差分对写型接口的误触（角色重放产生数据写入） | 中 | 差分默认仅对幂等读接口；写接口差分须 account-grant permitted_actions 显式覆盖+L3 逐条审批（§6.6 护栏） | 客户未声明写授权时认证后写漏洞只能报告为 C3 线索 |

---

**自检对照**：拍板四条→§0.1/§2/§3.2/§8.5/§10/§11；13 表字段齐全→§4（含 CB-1 口径裁定）；九门×phases.yaml 语义完整→§5.2；五宿主矩阵含验证方案→§10（含盲区 §10.3）；批次 0 接口清单完整→§11 批次 0 行（十六项）。本文档为唯一权威；后续修订走 schema_version+迁移命令+演进记录（docs/panorama 同步刷新）。
